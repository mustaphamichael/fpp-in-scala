...
object LineOfSight extends LineOfSightInterface {
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    ...
    def loop(index: Int, max: Float): Float = {
      if (index == until) max
      else {
        val tangent = input(index) / index
        if (tangent > max) loop(index + 1, tangent)
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    if (from < until) {
      ...
      output(from) = if (tangent > startingAngle) tangent else startingAngle
      downsweepSequential(input, output, tangent, from + 1, until)
    }
  }
...
// Sequential Time: 18.762436160000007 ms
// Parallel Time : 9.84163533 ms ; speedup: 1.9064348079233298