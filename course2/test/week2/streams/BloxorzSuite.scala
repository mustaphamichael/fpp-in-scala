...
class BloxorzSuite {
  ...
  @Test def `(local) test neighborsWithHistory`: Unit =
    new Level1 {
      ...
      assertEquals(neighbours.toSet, result)
    }

  @Test def `(local) test newNeighborsOnly`: Unit =
    new Level1 {
      val neighbors = newNeighborsOnly(Set(
        (Block(Pos(1, 2), Pos(1, 3)), List(Right, Left, Up)),
        (Block(Pos(2, 1), Pos(3, 1)), List(Down, Left, Up))
      ).to(LazyList), Set(Block(Pos(1, 2), Pos(1, 3)), Block(Pos(1, 1), Pos(1, 1)))
      )
      val result = Set((Block(Pos(2, 1), Pos(3, 1)), List(Down, Left, Up))).to(LazyList)
