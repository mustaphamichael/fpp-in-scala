...
trait DecoderInstances {
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromPartialFunction {
      case Json.Arr(as) => as.map(json => decoder.decode(json) match {
        case Some(value) => value
        case None => sys.error(s"Error decoding entity - $json")
      })
    }
  ...
  def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] =
    Decoder.fromPartialFunction { case Json.Obj(fields) =>
      decoder.decode(fields(name)) match {
        case Some(value) => value
        case None => sys.error(s"Error decoding field - $name")
      }
    }
}
