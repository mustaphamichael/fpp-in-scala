...
trait Solver extends GameDef {
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if (initial.isEmpty) LazyList.empty
      ...
      val more = for {
        ...
        //        if !explored.contains(neighbors._1)
      } yield newNeighbors
      //      println("abba", more.toSet)
  ...
  lazy val solution: List[Move] =
    if (pathsToGoal.isEmpty) List()
      ...
      // Traverse through the path list to find the path that matches the goal
      val result = for {
        (block, moves) <- pathsToGoal
        if done(block)
      } yield moves
      // Return an empty list, if no solution exists
      // else return the list in it's right order
      if (result.isEmpty) List() else result.head.reverse
    }
}
